...
namespace Microsoft.Performance.SDK.Runtime
{
	...
	public sealed class CustomDataSourceExecutor
	{
		...
		/// </param>
		public void InitializeCustomDataProcessor(ExecutionContext context, ILogger logger)
		{
				...
				table =>
				{
					...
					catch (Exception e)
					{
						lock (this.failedToEnableTables)
						{
							...
							logger?.Error("Unable to enable {0}: {1}", table, e);
						}
					}
				});
		}
			...
			CancellationToken cancellationToken, ILogger logger)
		{
			...
			catch (Exception e)
			{
				...
				metadataFailure = e;
				logger?.Error("Unable to build metadata tables for {0}: {1}");
			}
			...
			catch (Exception e)
			{
				...
				infoFailure = e;
				logger?.Error("Unable to get data source info for {0}: {1}", this.Processor, e);
			}
